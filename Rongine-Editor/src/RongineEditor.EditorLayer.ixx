...
export module RongineEditor.EditorLayer;
...
export enum class SketchToolType
{
	None = 0,
	Line,
	Rectangle,
...
export class EditorLayer :public Rongine::Layer
{
public:
	EditorLayer();
...
private:
	...
	void SaveSceneAs();
	void OpenScene();
	void OnBooleanOperation(Rongine::Operation op);
...
private:
	...
	//NURBS
	std::vector<Rongine::CADControlPoint> m_SplinePoints;//nurbs曲线控制�?
	int m_HoveredControlPoint = -1; // 当前鼠标悬停的控制点索引
	int m_SelectedControlPointIndex = -1;

	// --- Gizmo 撤销状�?---
	bool m_GizmoEditing = false; // 是否正在拖拽�?
	Rongine::TransformComponent m_GizmoStartTransform; // 拖拽开始时的快�?

	// --- 拉伸模式状�?---
	bool m_IsExtrudeMode = false;       // 是否处于拉伸模式
	float m_ExtrudeHeight = 0.0f;       // 当前拉伸高度
	glm::mat4 m_ExtrudeGizmoMatrix;     // Gizmo 的初始位置矩�?
	Rongine::Entity m_PreviewEntity;    // 用于显示拉伸预览的临时实�?
...
void EditorLayer::onUpdate(Rongine::Timestep ts)
{
	...
	// 视口尺寸变化：相机逻辑在主线程；FBO/纹理�?GL 资源在渲染线�?resize
	...
	if (m_IsSketchMode && m_SketchPlaneEntity)
	{
		...
		glm::vec3 planeNormal = glm::vec3(sc.SketchMatrix[2]); // Z轴法�?

		auto [mx, my] = ImGui::GetMousePos(); //屏幕绝对坐标，减�?Viewport 偏移
	...
	if (m_IsSketchMode && m_IsCursorOnPlane && m_CurrentTool != SketchToolType::None)
	{
		...
		case SketchToolType::Rectangle: {
			auto& sc = m_SketchPlaneEntity.GetComponent<Rongine::SketchComponent>();
			glm::vec3 right = glm::vec3(sc.SketchMatrix[0]); // X�?
			glm::vec3 up = glm::vec3(sc.SketchMatrix[1]); // Y�?
			...
			if (m_IsDrawing && ImGui::IsMouseClicked(ImGuiMouseButton_Right))
			{
				if (m_SplinePoints.size() >= 2)
				{
					...
					if (edgeShape)
					{
						...
						cad.SplinePoints = m_SplinePoints; // [关键] 保存点数�?
						cad.SplineDegree = 3;
...
void EditorLayer::onImGuiRender()
{
	...
	if (ImGui::BeginMenuBar()) {
		...
		if (ImGui::BeginMenu("Tools"))
		{
			...
			{
				OnBooleanOperation(Rongine::Operation::Cut);
			}

			if (ImGui::MenuItem("Boolean Fuse (Selected + Tool)"))
			{
				OnBooleanOperation(Rongine::Operation::Fuse);
			}

			if (ImGui::MenuItem("Boolean Common (Intersection)"))
			{
				OnBooleanOperation(Rongine::Operation::Common);
			}
	...
	if (useSpectral)
	{
		...
		if (ImGui::DragFloatRange2("Wavelength",
			&lambdaRange[0], &lambdaRange[1], // 当前�?
			1.0f,                             // 拖动速度
			380.0f, 780.0f,                   // 最�?最大限�?
	...
	ImGui::SetCursorPos(ImVec2(4, 4)); // 稍微向内偏移一�?
	...
	if (m_IsSketchMode)
	{
		...
		ImVec2 workPos = { viewportPos.x + 20, viewportPos.y + 50 }; // 左上�?偏移
	...
	static_cast<Rongine::ImGuiLayer*>(Rongine::Application::get().getImGuiLayer())->blockEvents(!m_viewportFocused && !m_viewportHovered);
	// 防止最小化时出现负数导致崩�?
		...
		if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("SPECTRAL_MAT_ITEM"))
		{
			...
			if (isValidEntity)
			{
				...
				if (Rongine::SpectralAssetManager::GetPreset(matName, preset))
				{
					...
					//  根据 Type 自动配置物理属�?(Metallic/Roughness)
					if (targetEntity.HasComponent<Rongine::MaterialComponent>())
					{
						...
						{
							// 玻璃/水：无金属度，极其光滑，Albedo 设为�?因为颜色主要�?Transmission)
							mat.Metallic = 0.0f;
							mat.Roughness = 0.0f;
	...
	if (m_IsExtrudeMode)
	{
		...
		// 2. 准备 Gizmo 矩阵
		// 我们需要把 Gizmo 放在 m_ExtrudeGizmoMatrix 的位�?
		// 并且我们把当前的拉伸�?m_ExtrudeHeight 叠加�?Z 轴上显示

		glm::mat4 currentGizmoMatrix = glm::translate(m_ExtrudeGizmoMatrix, glm::vec3(0, 0, m_ExtrudeHeight));

		// 3. 绘制 Gizmo (只显�?Z 轴平�?
		// 使用 ImGuizmo::TRANSLATE_Z 可以限制只沿 Z 轴移�?
		// 注意：ImGuizmo �?Z 轴对应我们的法线方向
		...
		{
			// 计算相对于初始位置的偏移�?
			// currentGizmoMatrix 的第 4 �?(Translation) �?m_ExtrudeGizmoMatrix 的差�?
			// 或者更简单：我们只需要知道这次操作移动了多少

			// 为了简单，我们反解 Z 轴的移动�?
			// (这里简化处理：假设只操作了 Translate Z)

			// 更稳健的方法：计�?currentGizmoMatrix 原点 �?m_ExtrudeGizmoMatrix 原点的距离，并投影到 Z �?
			glm::vec3 startPos = m_ExtrudeGizmoMatrix[3];
			glm::vec3 currentPos = currentGizmoMatrix[3];
			glm::vec3 normal = m_ExtrudeGizmoMatrix[2]; // Z轴方�?

			// 投影向量 (Current - Start) �?Normal �?
			float newHeight = glm::dot(currentPos - startPos, glm::normalize(normal));

			// 如果高度变了，更新预�?
			if (std::abs(newHeight - m_ExtrudeHeight) > 0.01f) // 加个阈值减少计算频�?
			{
				...
				{
					// 同样要变换到世界坐标�?
					// 注意：ExtrudeFace 生成的是局部形状，我们需要给 PreviewEntity 设置正确�?Parent Transform
		...
		if (ImGuizmo::IsUsing())
		{
			...
			if (std::abs(newRadius - m_FilletRadius) > 0.01f)
			{
				...
				if (previewShape)
				{
					...
					delete (TopoDS_Shape*)previewShape; // 只是预览，用完即�?
				}
			}
		}
	...
	if (isSplineSelected && m_SelectedControlPointIndex != -1 && m_gizmoType != -1)
	{
		...
		{
			// 1. 构�?Gizmo 矩阵 (Local -> World)
			// 我们的点是相对于物体的，所�?点的世界坐标 = 物体变换 * 点的局部位�?
	...
	if (m_selectedEntity && m_gizmoType != -1)
	{
		...
		// 如果是参数化模型，且当前处于缩放模式 (R�?，则强制不显�?Gizmo，也不允许操�?
		if (isParametricCAD && m_gizmoType == ImGuizmo::SCALE)
		{
			// 你可以选择什么都不做，或者在屏幕上画个提�?
			// 这里�?continue/return 取决于你的代码结构，如果是单独的函数直接 return
			// 简单做法：�?gizmoType 临时设为 -1 (隐藏)，或者直接跳过下面的 Manipulate 调用
			// 但为了不破坏状态，最简单的办法是：什么都不画，直接跳过本�?Gizmo 逻辑

			// 跳出 Gizmo 逻辑�?(假设这是在一个大 if 里，或者你可以把下面的逻辑包在一�?else �?
			goto SkipGizmo;
		}
		...
		{
			auto& mesh = m_selectedEntity.GetComponent<Rongine::MeshComponent>();
			// 确保我们�?Step 1 中存�?LocalVertices 不为�?
			if (!mesh.LocalVertices.empty())
			{
				...
				// C. 强制修改 Gizmo 矩阵的位置列 (�?�?，让 Gizmo 跳到面中�?
				// 注意：我们只改位置，不改旋转和缩放，所�?Gizmo 看起来方向还是正�?
		...
		else
		{
			...
			if (m_GizmoEditing)
			{
				...
				{
					// 创建并压入命�?
					// 注意：Execute() 会再次设置一�?Transform，但这没关系，因为数据是一样的
	...
	dispatcher.dispatch<Rongine::KeyPressedEvent>([this](Rongine::KeyPressedEvent& event) -> bool
	{
		...
		if (event.getKeyCode() == Rongine::Key::F)
		{
			if (m_selectedEntity && m_selectedEntity.HasComponent<Rongine::MeshComponent>())
			{
				...
				// 3. 计算变换后的 AABB 中心�?(作为新的焦点)
				// 注意：这里做了一个简单的假设，物体中�?= 变换后的 AABB 中心
				// 更严谨的做法是变�?AABB �?8 个顶点再求新�?min/max
				...
				float fovRad = glm::radians(45.0f); // 假设相机 FOV �?45 �?
...
void EditorLayer::OnBooleanOperation(Rongine::Operation op)
{
	// 检查有效性：必须选中了物体，且工具物体也存在，且两者不是同一�?
	...
	if (resultShapeHandle)
	{
		...
		*occShape = centeredShape; // 简单粗暴覆�?
		...
		// 添加 Transform
		// 注意：因�?Perform 已经在世界坐标系下运算了，所以新物体�?Transform 应该是归零的
		...
		// 添加 CAD 组件 (类型设为 Imported，因为它已经不是纯参数化的球/方块�?
		...
		if (va)
		{
			...
			// ==================== 生成边框�?====================
			std::vector<Rongine::LineVertex> lineVerts;
			// 这里如果�?cadComp，最好用 cadComp.LinearDeflection，这里简化用 0.1f 或继�?
			auto edgeVA = Rongine::CADMesher::CreateEdgeMeshFromShape(*occShape, lineVerts, meshComp.m_IDToEdgeMap, 0.1f);
			meshComp.EdgeVA = edgeVA;
			meshComp.LocalLines = lineVerts;
			meshComp.LocalIndices = indices;
...
void EditorLayer::EnterExtrudeMode()
{
	...
	// 1. 获取选中面的坐标系，作为 Gizmo 的初始位�?
	// 注意：这个矩阵是局部空间的还是世界空间的？
	// GetFaceTransform 返回的是局部坐标系下的点�?
	// 我们需要把它转到世界坐标系，以�?ImGuizmo 正确显示�?
...
void EditorLayer::ExitExtrudeMode(bool apply)
{
	...
	if (apply && std::abs(m_ExtrudeHeight) > 0.001f)
	{
		...
		if (toolShapePtr)
		{
			...
			// 逻辑 A: 尝试寻找父物�?(例如：如果是在一个立方体的面上画的草图，sourceEntity 就是那个立方�?
			...
			// 2. 基底形状确实是一个实�?(SOLID/COMPSOLID)，而不是面
			// 3. 我们不是在拉伸一个独立的草图�?(除非你想把它消耗掉)
				...
				(baseShape->ShapeType() == TopAbs_SOLID || baseShape->ShapeType() == TopAbs_COMPSOLID))
			{
				try
				{
					...
					// 这里假设 ExtrudeFace 已经在世界空间或�?Base 对齐的空间生成了形状�?
			...
			if (!performedBoolean)
			{
				...
				newCad.ShapeHandle = new TopoDS_Shape(*toolShape); // 深拷贝形�?
			...
			delete (TopoDS_Shape*)toolShapePtr; // 清理临时�?Tool Shape 指针
		}
	}
...
void EditorLayer::EnterFilletMode()
{
	...
	if (m_selectedEntity.HasComponent<Rongine::MeshComponent>())
	{
		...
		//复制srcMesh，因为操作AddComponent后，内存池可能会满，会重新申请一片内存，释放原来的内存扩�?
...
void EditorLayer::ExitFilletMode(bool apply)
{
	...
	m_FilletRadius = 0.0f;
	m_FilletEdge = -1;
	m_selectedEdge = -1; // 倒角�?Edge ID 会变，必须重置选择
}
...
void EditorLayer::EnterSketchMode()
{
	...
	if (Rongine::CADFeature::GetPlanarFaceCoordinateSystem(*(TopoDS_Shape*)cad.ShapeHandle, m_selectedFace, sketchAx3, sketchMat))
	{
		...
		glm::vec3 normal = glm::vec3(sketchMat[2]); // 草图法线 (Z�?
...
void EditorLayer::buildRenderGraph()
{
			...
			for (auto entityHandle : view)
			{
				...
				if (mesh.VA)
				{
					if (m_selectedEntity == entityHandle && mesh.EdgeVA)
					{
						...
						// (暂保留直�?GL 调用，后续可移入 PipelineState)
	...
	// ==========================================
	//  Pass 2: Wireframe / Overlay Pass (线框、网格、草�?
	// ==========================================
	{
		Rongine::RenderPassSpec spec;
