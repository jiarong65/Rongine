...
export class Sandbox2D:public Rongine::Layer
{
public:
	Sandbox2D();
...
void Sandbox2D::onImGuiRender()
{
	...
	ImGui::PopStyleVar();
	ImGui::PopStyleVar(2); // 弹出之前 Push 的两�?StyleVar
	...
	ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2{ 0, 0 }); // 让图片铺满窗�?
	ImGui::Begin("Viewport");

	// 拿到 FBO 里的纹理 ID
	textureID = m_framebuffer->getColorAttachmentRendererID();
	// �?FBO 的内容绘制到 ImGui 窗口�?
	// 注意：OpenGL �?UV 坐标�?y 轴向上，ImGui 是向下，所以需要翻�?UV：{0, 1}, {1, 0}
