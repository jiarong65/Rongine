...
export module Rongine.GeometryUtils;

export import Rongine.GeometryData;
...
export namespace Rongine {
	...
	class GeometryUtils {
	public:
		// majorRadius: torus major radius, minorRadius: tube radius
	...
	Ref<VertexArray> GeometryUtils::CreateTorus(float majorRadius, float minorRadius, int majorSegments, int minorSegments) {
		...
		for (int i = 0; i <= majorSegments; ++i) {
			...
			for (int j = 0; j <= minorSegments; ++j) {
				...
				vertex.Color = { 1.0f, 0.4f, 0.4f, 1.0f };
	...
	FaceInfo GeometryUtils::CalculateFaceCenter(const std::vector<CubeVertex>& vertices, int targetFaceID)
	{
		...
		if (count > 0)
		{
			info.LocalCenter = sumPos / (float)count;
			info.LocalNormal = glm::normalize(sumNormal / (float)count);
		}
		else
		{
